...
def get_tiles(img, tsize, padding, seed):
    ty=tx=0
    result = []
    h,w = img.shape[:2]
    tt = tsize    
    
    ih = ((h+tsize-1)//tsize)*tsize
    iw = ((w+tsize-1)//tsize)*tsize    
    img = np.pad(img, ((padding, ih-h+padding),(padding,iw-w+padding),(0,0)), 'constant', constant_values=0)       
    
        
    while ty<ih-tsize-2*padding:
        tx=0
        while tx<iw-tsize-2*padding:
            th, tw = min(2*padding+ty+tsize, ih), min(2*padding+tx+tsize,iw)            
            ptile = img[ty:th, tx:tw,:]                                 
            result.append(ptile)
            tx+=tsize//2
        ty+=tsize//2
    #print(len(result))
    ...
    for i in range(20):
        ...
        y0 = seed[i][1] #np.random.randint(0,h-tt)
        tile = img[x0:x0+tt+2*padding, y0:y0+tt+2*padding,:]
        result.append(tile)
    ...
    for root, dirs, files in os.walk(dataset_path, topdown=False):
        for name in files:            
            ...
            if name.endswith(".png") or name.endswith('.jpg'):
                ...
                seed = [(np.random.randint(0,img_in.shape[1]-80), np.random.randint(0,img_in.shape[0]-80)) for _ in range(20)]                
                
                #print(img_in.shape)
                #print(img_out.shape)
                
                for ti, to in list(zip(get_tiles(img_in, 64, 8, seed), get_tiles(img_out, 64, 8, seed))):
                    #print(ti.shape, to.shape)
                    yield ti, to[8:72,8:72]
