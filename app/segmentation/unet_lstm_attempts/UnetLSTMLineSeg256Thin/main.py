# C:\Users\Stefan\miniconda3\envs\kaggle\python.exe D:\Public\CNNLSTMLineSeg256Thin\main.py
...
def main():       
    ...
    ds = dataset_generator(ds, (80,80), (64,64))
    
    """
    i=0
    for x,y in list(itertools.islice(ds,0,100)):     
        plt.imshow(np.concatenate([x,y],axis=1))
        plt.savefig(f"D:\\Public\\CNNLSTMLineSeg256Thin\\figures\\{i}.png")        
        plt.close()
        i+=1       
    """
    ...
    def create_segmentation_model():
        def enc_block(l, units):
            ...
            l = tf.keras.layers.Dropout(0.5/units)(l)
            return l
            
        def dec_block(l1, l2, units, margin=0):
            ...
            if margin>0:
                l2 = tf.keras.layers.Cropping2D(margin)(l2)
        ...
        x = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(2, return_sequences=True, dropout=0.05))(x)        
        x = tf.keras.layers.Reshape((2,2,64))(x)    
               
        #x = dec_block(x, x6, 128)
        x = dec_block(x, x5, 64)
        x = dec_block(x, x4, 32, 1)
        x = dec_block(x, x3, 16, 2)
        x = dec_block(x, x2, 8, 4)
        x = dec_block(x, x1, 4, 8)
    ...
    def callback_plot(epoch, logs):
        if epoch % 1 == 0:
            ...
            k=0
            for x, _y0 in list(itertools.islice(q_ds,0, n-1)) + list(itertools.islice(q_testds, 0, 1)):
                x = x.numpy()                                        
                x = x.reshape(1, *TIMG_SIZE, 1)                         
                
                y = np.zeros((80,80,1))
                y[8:72,8:72,:] = model.predict(x)[0]
            ...
            plt.savefig(f"D:\\Public\\CNNLSTMLineSeg256Thin\\figures\\ctc_{epoch}_{logs['loss']}_{logs['accuracy']}.png")
            plt.close()       
            
        
        if epoch % 2 == 0:                
            #save_opt(train_model, "D:\\Public\\CNNLSTMFullPage\\train_model_0")
            model.save("D:\\Public\\CNNLSTMLineSeg256Thin\\train_model_0")



    #history = model.fit(ds, epochs=100, steps_per_epoch=400,
    history = model.fit(ds, epochs=100, steps_per_epoch=50,
    ...
    plt.legend()
    plt.savefig("D:\\Public\\CNNLSTMLineSeg256Thin\\plot_acc.png")
    plt.close()
    plt.plot(history.history['loss'], label="loss")
    plt.plot(history.history['val_loss'], label="val_loss")
    plt.legend()
    plt.savefig("D:\\Public\\CNNLSTMLineSeg256Thin\\plot_loss.png")
...
def disp():    
    ...
    gen = dataset_generator(ds, (80,80))
