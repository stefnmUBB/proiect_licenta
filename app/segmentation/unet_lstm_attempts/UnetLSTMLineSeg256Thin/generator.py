...
def generator(img_floats_iter):   
    for i,o in img_floats_iter:        
        i = tf.constant(i)
        o = tf.constant(o)        
        
        if tf.reduce_sum(o)==0:            
            if np.random.rand()>0.1:
                continue
        
        yield (i,o)

def dataset_generator(img_floats_iter, img_size, img_size2):
    return tf.data.Dataset.from_generator(lambda: generator(img_floats_iter),
                                          output_types=(tf.float32, tf.float32),
                                          output_shapes=(img_size+(1,), img_size2+(1,)))
