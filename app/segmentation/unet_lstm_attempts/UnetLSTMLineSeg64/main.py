# C:\Users\Stefan\miniconda3\envs\kaggle\python.exe D:\Public\CNNLSTMLineSeg64\main.py
...
def main():   
    ...
    def create_segmentation_model():
        ...
        x = tf.keras.layers.Reshape((128,4))(x)    
    ...
    def dice(y_true, y_pred):                   
        t1 = tf.greater(y_true, 0.5)        
        t2 = tf.greater(y_pred, 0.5)              
        tp = tf.reduce_sum(tf.cast(tf.logical_and(t1, t2), "float32"))        
        t = tf.multiply((y_true-0.5),(y_pred-0.5))        
        fpfn = tf.reduce_sum(tf.cast(tf.greater(tf.zeros_like(t), t), "float32"))                
        s = 1-tf.cast(tf.greater(tp*fpfn, 0), "float32") 
        return 2*(tp+s)/(2*(tp+s)+fpfn)
    ...
    def callback_plot(epoch, logs):
        ...
        if epoch % 2 == 0:                
            ...
            model.save("D:\\Public\\CNNLSTMLineSeg64\\train_model_0")
    ...
    def plot_metric(metric_name, val=True, log_scale=False):
        if(log_scale):
            plt.yscale("log")
        else:
            plt.yscale("linear")
        ...
        plt.legend()
        plt.savefig(f"D:\\Public\\CNNLSTMLineSeg64\\{metric_name}.png")
        plt.close()

    plot_metric("accuracy")
    plot_metric("loss", log_scale=True)
    plot_metric("precision")
    plot_metric("recall")
    plot_metric("IoU")
    plot_metric("dice")
    plot_metric("lr", val=False)
    exit(0)
