...
from model import create_recognition_model, create_train_model
...
def callback_plot(epoch, logs):
    if epoch % 1 == 0:
        ...
        for i in range(1):
            ...
            pred = out_pred[i]
            ptxt = "".join([characters[c] for c in y[i] if c < len(characters)])[:50]+"..."
        ...
        plt.savefig(f"D:\\Public\\CNNLSTMFullPage\\figures\\ctc_{epoch}.png")
        plt.close()
        ...
        with open(f"D:\\Public\\CNNLSTMFullPage\\figures\\prd_{epoch}.txt","w",encoding="utf8") as f:
            for i in range(len(d)):
                ...
                ptxt = "".join([characters[c] for c in y[i] if c < len(characters)])
                f.write(f"Real: {true}\r\nPredicted: {ptxt}\r\n\r\n")

    if epoch % 2 == 0:
        recognition_model.save_weights("D:\\Public\\CNNLSTMFullPage\\model_lstm_fullscale.weights.h5")
