...
namespace LillyScan.Backend.Math
{
    internal static class Pooling
    {
        ...
        public static unsafe void MaxPooling2DValidPadding(float[] a, float[] r, int B, int N, int M, int C,
         int poolSizeRows, int poolSizeCols, int stridesRows, int stridesCols)
        {
            fixed (float* pa = &a[0])
            fixed (float* pr = &r[0])
                MaxPooling2DValidPadding(pa, pr, B, N, M, C, poolSizeRows, poolSizeCols, stridesRows, stridesCols);
        }

        public static unsafe void MaxPooling2DSamePadding(float[] a, float[] r, int B, int N, int M, int C,
            int poolSizeRows, int poolSizeCols, int stridesRows, int stridesCols)
        {
            fixed (float* pa = &a[0])
            fixed (float* pr = &r[0])
                MaxPooling2DSamePadding(pa, pr, B, N, M, C, poolSizeRows, poolSizeCols, stridesRows, stridesCols);
        }
