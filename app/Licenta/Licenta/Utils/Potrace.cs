...
namespace Licenta.Utils
{
    ...
    public static class Potrace
    {
        ...
        {
            Console.WriteLine("Potrace");
            options = options ?? new PotraceOptions();

            var scale = 1;
            var allCorners = new List<Corner[]>();
            using (var bmp = new Bitmap(m.ColumnsCount * scale, m.RowsCount * scale, System.Drawing.Imaging.PixelFormat.Format32bppPArgb))
            {
        ...
        {
            new ImageRGB(matrix).ToBitmap().Save("result.png");
            ...
            {
                if (x == 0) border0++; else border1++;
                return true;
            });            
            var mat = Matrices.DoEachItem(matrix, i => (byte)((i > 0 ? 0xFF : 0x00) ^ (border1 > border0 ? 0xFF : 0x00)));
            int by = 0, bx = 0;
        ...
        private static Vertex NextVertex(Matrix<byte> m, Vertex v, Path path)
        {
            ...
            {
                if (!path.Vertices.Any(w => w.Y == candidates[0].Y && w.X == candidates[0].X))
                    return candidates[0];
                if (!path.Vertices.Any(w => w.Y == candidates[1].Y && w.X == candidates[1].X))
                    return candidates[1];
                var rand = new Random();
                return candidates[rand.Next() % 2]; // prevent getting stuck in infinite cycles                
            }
        ...
        static List<int> FindPolygon(Path path, out E e)
        {
            ...
            var possibleSegments = new Dictionary<int, HashSet<int>>();
            {
                ...
                //Console.WriteLine($"Possible segments count = {pscount}");
            }            
            ...
            var R = possibleSegments; // ReverseGraph(possibleSegments);
            ...
            Console.WriteLine($"Final cost/pl: {rCost} {rPath.Count}");            
